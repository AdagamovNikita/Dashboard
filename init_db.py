...
Base = declarative_base() #it creates a base class for all tables that I will use
engine = create_engine('sqlite:///store.db') #just a connection to a database 
Session = sessionmaker(bind=engine) #we need it to create sessions. session is like a cursor in raw sql
