...
def search_brand():
    try:
        ...
        results = conn.execute('''
            SELECT * FROM SearchBrandView WHERE Brand = ?
        ''', (brand,)).fetchall()
...
def category_details():
    try:
        ...
        if not conn:
            return jsonify({"error": "Sorry, there is  a server problem :("})
        categories = conn.execute('''
            SELECT * FROM CategoryDetailsView
        ''').fetchall()
...
def get_sales_data():
    try:
        ...
        return jsonify([{
            ...
            'sales': row['TotalSales'] / 100  # Convert cents to dollars/euros
        } for row in results])
    except Exception as e:
        print(f"Error: {e}")
        return jsonify({"error": "Sorry, there is a server problem :("})
